...
public class GPTChatbot : MonoBehaviour
{
    ...
    {
        if (awaitingResponse && !(message.Contains("응") || message.Contains("네") || message.Contains("정지") || message.Contains("멈춰")))
        {
            AddBotBubble("현재 대기 중입니다. '정지' 또는 '응'으로 응답해 주세요.");
            yield break;
        }
    ...
    private void HandleAction(string action)
    {
        ...
        switch (action)
        {
            case "정지": amr.enabled = false; AddBotBubble("AMR을 정지했습니다."); break;
            case "시작": amr.enabled = true; AddBotBubble("AMR을 다시 시작했습니다."); break;
            case "진단": StartCoroutine(SendChatRequest("진단 요청", ComposePrompt("진단 요청", false))); break;
            case "수동":
                amr.enabled = false;
                var manual = amr.GetComponent<ARMMovement>();
                if (manual != null) manual.enabled = true;
